...
const calculateEnergyUsageSimple = (profile) => {
  ...
  for (const e of profile.events) {
    ...
    if(lastState === 'on'){
      usage += e.timestamp - onTimeStamp;
    }else{
      onTimeStamp = e.timestamp;
    }
  ...
  if(lastState === 'on'){
    usage += MAX_IN_PERIOD - onTimeStamp;
  }
...
const calculateEnergySavings = (profile) => {
  ...
  for (const e of profile.events) {
    if(e.state === lastState || (e.state === 'off' && lastState === 'auto-off')){
      continue;
    }

    if(e.state === 'auto-off'){
      autoOffTimeStamp = e.timestamp;
    }
    if(lastState === 'auto-off'){
      usage += e.timestamp - autoOffTimeStamp;
    }
  ...
  if(lastState === 'auto-off'){
    usage += MAX_IN_PERIOD - autoOffTimeStamp;
  }
